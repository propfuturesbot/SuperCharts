...
const StrategyManagement = () => {
  ...
  const handleStartStop = async (strategyId, currentStatus) => {
    ...
    try {
      await axios.patch(`http://localhost:8026/api/strategies/${strategyId}`, { status: newStatus });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:8026/api/strategies/${strategyToDelete.id}`);
  ...
  const handleClone = async (strategy) => {
    ...
    try {
      const response = await axios.post('http://localhost:8026/api/strategies', clonedStrategy);

...
if (isCloudflare) {
  // When accessed through Cloudflare tunnel, use the same hostname but port 8025
  // We'll use a proxy approach - backend needs to be accessible
  // For now, we'll use the tunnel URL's origin + /api prefix
  API_BASE_URL = window.location.origin;
} else if (isLocalhost) {
  // Local development - backend is on port 8025
  API_BASE_URL = 'http://localhost:8025';
} else {
  // Fallback - assume backend is on same host, port 8025

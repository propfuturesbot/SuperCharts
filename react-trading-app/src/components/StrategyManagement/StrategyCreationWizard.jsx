...
const StrategyCreationWizard = ({ isOpen, onClose, onStrategyCreated, editMode = false, strategy = null }) => {
  ...
  const lookupContractProductId = async (contractName) => {
    try {
      const response = await axios.get(`http://localhost:8026/api/contracts/lookup/${encodeURIComponent(contractName)}`);
  ...
  const fetchContracts = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:8026/api/contracts');
  ...
  const handleSaveStrategy = async (chartData = null) => {
    ...
    try {
      ...
      if (editMode && strategy?.id) {
        ...
        try {
          const response = await axios.put(`http://localhost:8026/api/strategies/${strategy.id}`, strategyData);
          updatedStrategy = {
            ...strategyData,
            id: strategy.id,
        ...
        try {
          const response = await axios.post('http://localhost:8026/api/strategies', strategyData);
          updatedStrategy = {
            ...strategyData,
            id: response.data.id,
            status: 'inactive',
  ...
  const launchChart = async (strategyId = null) => {
    try {
      ...
      const response = await axios.post('http://localhost:8026/api/launch-chart', chartConfig);

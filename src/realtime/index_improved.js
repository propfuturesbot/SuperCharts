// Authentication and provider configuration will be loaded dynamically from centralized auth system
let ACCESS_TOKEN = null;
let CURRENT_PROVIDER = 'topstepx';
let PROVIDER_CONFIG = null;
let CURRENT_USERNAME = null;
let CURRENT_SYMBOL = 'F.US.MNQ'; // Default symbol, will be configurable
...
// Configuration management - now dynamic instead of hardcoded
const createResolutionConfig = (symbol = 'F.US.MNQ') => ({
  // Tick-based resolutions
  '100T': { countback: 50, displayName: '100 Ticks', symbol: symbol },
  '500T': { countback: 50, displayName: '500 Ticks', symbol: symbol },
  '1000T': { countback: 50, displayName: '1000 Ticks', symbol: symbol },
  '5000T': { countback: 50, displayName: '5000 Ticks', symbol: symbol },

  // Second-based resolutions
  '1S': { countback: 500, displayName: '1 Second', symbol: symbol },
  '5S': { countback: 500, displayName: '5 Seconds', symbol: symbol },
  '10S': { countback: 500, displayName: '10 Seconds', symbol: symbol },
  '15S': { countback: 500, displayName: '15 Seconds', symbol: symbol },
  '20S': { countback: 500, displayName: '20 Seconds', symbol: symbol },
  '30S': { countback: 500, displayName: '30 Seconds', symbol: symbol },

  // Minute-based resolutions
  '1': { countback: 500, displayName: '1 Minute', symbol: symbol },
  '2': { countback: 500, displayName: '2 Minutes', symbol: symbol },
  '3': { countback: 500, displayName: '3 Minutes', symbol: symbol },
  '4': { countback: 500, displayName: '4 Minutes', symbol: symbol },
  '5': { countback: 500, displayName: '5 Minutes', symbol: symbol },
  '10': { countback: 500, displayName: '10 Minutes', symbol: symbol },
  '15': { countback: 500, displayName: '15 Minutes', symbol: symbol },
  '20': { countback: 500, displayName: '20 Minutes', symbol: symbol },
  '30': { countback: 500, displayName: '30 Minutes', symbol: symbol },
  '45': { countback: 500, displayName: '45 Minutes', symbol: symbol },
  '60': { countback: 500, displayName: '1 Hour', symbol: symbol },
  '1D': { countback: 326, displayName: '1 Day', symbol: symbol },
  '1W': { countback: 500, displayName: '1 Week', symbol: symbol },
  '1M': { countback: 500, displayName: '1 Month', symbol: symbol }
});

// Dynamic resolution config that updates with current symbol
let resolutionConfig = createResolutionConfig(CURRENT_SYMBOL);
...
const importAuthService = async () => {
  try {
    // Import the auth service from the React app
    const authModule = await import('../../react-trading-app/src/services/auth.service.js');
...
const importProviderConfig = async () => {
  try {
    // Import the provider config from the React app
    const providerModule = await import('../../react-trading-app/src/config/providers.js');
    return providerModule.getProviderConfig;
  } catch (error) {
...
// Function to update symbol and regenerate config
const updateSymbol = (newSymbol) => {
  CURRENT_SYMBOL = newSymbol;
  resolutionConfig = createResolutionConfig(CURRENT_SYMBOL);
  console.log(`Updated symbol to: ${CURRENT_SYMBOL}`);
};
...
const getHistoricalData = async (resolution, countback, symbol = null) => {
  try {
    ...
    // Use current symbol or provided symbol
    const targetSymbol = symbol || encodeURIComponent(`/${CURRENT_SYMBOL.replace('F.US.', '')}`);

    const now = Math.floor(Date.now() / 1000);
    const from = now - (86400 * 7); // 7 days ago
    const to = now;

    const url = `${PROVIDER_CONFIG.chartapi_endpoint}/History/v2?Symbol=${targetSymbol}&Resolution=${resolution}&Countback=${countback}&From=${from}&To=${to}&SessionId=extended&Live=false`;
...
// Initialize authentication using centralized auth system
const initializeAuth = async () => {
  console.log('🔐 Initializing authentication with centralized auth system...');
  ...
  try {
    ...
    CURRENT_PROVIDER = authService.getProvider() || 'topstepx';
    ...
    CURRENT_USERNAME = authService.getUsername();

    console.log('✅ Authentication successful');
    console.log(`👤 User: ${CURRENT_USERNAME || 'Unknown'}`);

    return true;
  } catch (error) {
    console.error('❌ Authentication failed:', error);
    updateStatus('Authentication Failed - Please log in', false);
    return false;
  }
};

const main = async () => {
  console.log('=== Initializing Improved Trading Chart Application ===');
...
const changeChartType = (chartType) => {
  ...
  const symbolDisplay = CURRENT_SYMBOL.replace('F.US.', '/');
  let title = `${symbolDisplay} Real-Time Chart - ${providerName}`;
  if (chartType === 'heikenashi') {
    title = `${symbolDisplay} Heiken Ashi Chart - ${providerName}`;
  } else if (chartType === 'renko') {
    title = `${symbolDisplay} Renko Chart (${currentBrickSize}) - ${providerName}`;
  }
...
const updateRenkoBrickSize = () => {
  try {
    ...
    const providerName = PROVIDER_CONFIG ? PROVIDER_CONFIG.name : 'Trading';
    const symbolDisplay = CURRENT_SYMBOL.replace('F.US.', '/');
    document.querySelector('h1').textContent = `${symbolDisplay} Renko Chart (${currentBrickSize}) - ${providerName}`;
